    ...
    getMatchingTabs(text, function (matchingTabs) {
      var suggestionText =
        matchingTabs.length < 1
          ...
          : matchingTabs.length +
            " tabs matching. Press enter to move them to a new window.";
  ...
  chrome.omnibox.onInputEntered.addListener(function (text) {
    getMatchingTabs(text, function (matchingTabs) {
      if (matchingTabs.length < 1 || text === "") {
        alert(
          'no matches found for the keywords "' +
            text +
            '".' +
            "\nNote: You do not need to press down to select a suggestion. Just press enter after entering keywords."
        );
      } else {
        chrome.windows.create({ type: "normal" }).then((win) => {
          var newWindow = win;
          chrome.tabs
            .move(matchingTabs, { windowId: newWindow.id, index: -1 })
            .then(() => {
